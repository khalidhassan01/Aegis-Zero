...
def test_network_can_be_disabled():
    v = engine(allow_network=False).decide("http_fetch", {"url": "https://example.com"})
    assert v.decision is Decision.DENY


@pytest.mark.parametrize(
    "path", ["/etc/shadow", "/etc/sudoers", "/proc/self/environ", "/root/.ssh/id_rsa"]
)
...
def test_secrets_are_redacted_by_key():
    v = engine().decide("http_fetch", {"url": "https://example.com", "api_key": "hunter2"})
...
def test_secrets_redacted_by_value_pattern():
    v = engine().decide(
        "http_fetch", {"url": "https://example.com", "note": "use sk-abcdefghijklmnopqrstu now"}
    )
...
def test_nested_secrets_are_redacted():
    v = engine().decide(
        "http_fetch", {"url": "https://example.com", "headers": {"authorization": "Bearer abc"}}
    )
...
def test_denied_tools_configuration():
    assert (
        engine(denied_tools=("write_file",)).decide("write_file", {"path": "/tmp/x"}).decision
        is Decision.DENY
    )
...
def test_per_run_call_limit():
    e = engine(rules={"search": PolicyRule(Risk.LOW, Decision.ALLOW, max_calls_per_run=2)})
