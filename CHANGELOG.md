...
- **Zero, the mascot, and a full brand kit.** `docs/brand/` ships the
  avatar mark — a round shield whose body *is* the digit 0, with a calm
  face in the zero-ring and a gold spark earned only by verification — at
  every social/favicon size (`zero-avatar-{512,256,128,64,32}.png`,
  `favicon.ico`), plus a horizontal lockup, a 1200×630 Open Graph card,
  and the video end-card plate. Rendered from the SVG masters by
  `scripts/make_brand.py`, pinned byte-for-byte by
  `docs/brand/brand.assets.json` + `tests/test_brand.py` (round-crop
  safety and favicon readability are tested, and the four masters cannot
  drift apart). The mascot's identity, palette, and usage rules live in
  `docs/brand/BRAND.md`.
- **Socials-ready demo video.** `docs/demo.mp4` (1080p, silent, 34 s — for
  X, Reddit, Show HN) and `docs/demo.gif` render the pinned demo timeline
  into shareable video: typewritten command, palette-coloured verdicts, a
  blinking closing cursor, and a branded end card — Zero above the
  wordmark — with the install command. Generated by
  `scripts/make_video.py` from the same transcript and schedule as the
  cast/SVG pair; `docs/demo.video.json` pins the shipped binaries
  byte-for-byte and `tests/test_demo_video.py` fails the build on drift,
  so all four demo assets can never disagree.
...
- 179 tests at 86% coverage at the time of the rebuild (342 today); four
  runnable examples.
