...
- **Socials-ready demo video.** `docs/demo.mp4` (1080p, silent, 34 s — for
  ...
  blinking closing cursor, and a branded end card with the install command.
  Generated by `scripts/make_video.py` from the same transcript and schedule
  as the cast/SVG pair; `docs/demo.video.json` pins the shipped binaries
  byte-for-byte and `tests/test_demo_video.py` fails the build on drift, so
  all four demo assets can never disagree.
