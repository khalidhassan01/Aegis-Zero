...
class EchoProvider(LLMProvider):
    ...
    def __init__(
        self, script: Sequence[Completion | str] | None = None, vector_size: int = 768
    ) -> None:
    ...
    async def complete(
        ...
        if self.script:
            ...
            if isinstance(nxt, str):
                return Completion(
                    text=nxt, model=model, usage=Usage(len(str(messages)) // 4, len(nxt) // 4)
                )
            return nxt
        last = messages[-1].content if messages else ""
        return Completion(
            text=f"echo: {last}", model=model, usage=Usage(len(last) // 4, len(last) // 4)
        )
    ...
    async def embed(self, texts: Sequence[str], *, model: str) -> list[list[float]]:
        ...
        for text in texts:
            ...
            vec = [((digest[i % len(digest)] / 255.0) - 0.5) for i in range(self.vector_size)]
            norm = sum(v * v for v in vec) ** 0.5 or 1.0
            out.append([v / norm for v in vec])
        return out


def scripted_tool_call(name: str, arguments: dict[str, Any], model: str = "test") -> Completion:
